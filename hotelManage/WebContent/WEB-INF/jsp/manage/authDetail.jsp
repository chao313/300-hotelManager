...
<!--                            <a  href="添加用户.html" class="label label-warning" style="float:right; margin:8px;">添加</a> -->
                        ...
                        <div class="well brown">
                            ...
                            <div >
                                         <div>用户名：${book.bookUser.name}</div>
                                         <div>密码：${manage.passWord}</div>
                                         <div>邮箱：${manage.email}</div>
                                         <div>性别：<c:if test="${manage.sex == 1 }"> 
								                                               男 
								               </c:if> 
 								               <c:if test="${manage.sex == 2 }">
								                                               女 
 								               </c:if> </div>
                                         <div>手机号码：${manage.phone}</div>
                                         <div>证件号：${manage.idCard}</div>
                                         <div>角色：  <c:if test="${manage.type == 1 }">
								                                              超级管理员
								               </c:if>
								               <c:if test="${manage.type == 0 }">
								                                              普通管理员
								               </c:if></div>
                                         <div>创建时间：<fmt:formatDate value="${manage.createTime }" pattern="yyyy-MM-dd HH:mm:ss"/></div>
                                         <div>状态：<c:if test="${manage.isDelete == 1 }"> 
								                                              已刪除 
 								               </c:if> 
